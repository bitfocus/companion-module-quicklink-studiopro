...
export function getActions() {
	...
	actions['toggle_dsk_item'] = {
		name: 'Set DSK',
		description: 'Set or toggle a DSK item',
		options: [
			{
				type: 'dropdown',
				label: 'DSK Tab',
			...
			},
			{
				...
				id: 'dskItem',
				default: "",
				choices: this.dskItemChoices,
				allowCustom: true,
				//isVisible: (options) => options.dskTab == '1',
				//isVisible: (options) => 2 === this.updateDSKITems(parseInt(options.dskTab))
			},
			{
				type: 'dropdown',
				label: 'Visible',
		...
		subscribe: async (action) => {
			let dskTab = await this.parseVariablesInString(action.options.dskTab)
			if (isNaN(dskTab || dskTab === undefined || dskTab === null || dskTab === "")) return undefined;
			if (isNaN(dskTab || dskTab === undefined || dskTab === null || dskTab === "")) return undefined;

			const dskTabIdx = parseInt(dskTab) - 1;
			if (dskTabIdx < 0) return;

			this.buildDSKItems(dskTabIdx);
		},
		/*unsubscribe: async (action) => {
			...
			this.log("info", "xxxxxxxxxxxxxxxxxxx");
